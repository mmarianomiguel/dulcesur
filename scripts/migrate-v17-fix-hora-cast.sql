-- migrate-v17-fix-hora-cast.sql
-- Fix: p_hora TEXT → explicit ::time cast to avoid
-- "column hora is of type time without time zone but expression is of type text"
...
BEGIN
  ...
  VALUES (gen_random_uuid(), v_numero, p_client_id, p_fecha::date, p_hora::time, p_monto, p_forma_pago, p_observacion, p_cuenta_bancaria_id, 'aplicado')
  ...
  VALUES (
    p_fecha::date, p_hora::time, 'ingreso',
...
BEGIN
  ...
  IF p_registrar_caja AND p_forma_pago <> 'Cuenta Corriente' THEN
    INSERT INTO caja_movimientos (fecha, hora, tipo, descripcion, metodo_pago, monto, referencia_id, referencia_tipo, cuenta_bancaria)
    VALUES (
      p_fecha::date, p_hora::time, 'egreso',
